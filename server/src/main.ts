...
const instance = jsonDiffPatch.create({
    objectHash: (obj: any, idx: number) => {
        ...
        if (has(obj, "id") && has(obj, "__typename")) {
            // console.log("returning", `${obj.__typename}:${obj.id}`)
            return `${obj.__typename}:${obj.id}`
        } else if (has(obj, "id")) {
            // console.log("returning", obj.id)
            return obj.id
        } else if (has(obj, "__ref")) {
            // console.log("returning", obj.__ref)
            return obj.__ref
        } else {
            const newObj = chain(obj).toPairs().sortBy(0).fromPairs().value()
...
if (patch != null) {
    ...
    result = instance.patch(cloneDeep(t0), patch)
}

console.log("result", JSON.stringify(result, null, 2))
console.log("equal?", isEqual(result, t1))
